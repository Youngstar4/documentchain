...
# dms_hash
RUN git clone https://github.com/Krekeler/documentchain_hash
RUN cd dms_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} dms
RUN useradd -u ${USER_ID} -g dms -s /bin/bash -m -d /dms dms
...
RUN mkdir /dms-src && \
  ...
  chown $USER_ID:$GROUP_ID /dms-src && \
