...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total dms balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized dms balance of the wallet\n"
...
UniValue devgetinfo(const JSONRPCRequest& request)
{
  //return "my dev comment";
    return FormatVersion(CLIENT_VERSION) + " release 2";
}
...
#ifdef ENABLE_WALLET
        // Case 1: DMS address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are dms addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) dms address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
