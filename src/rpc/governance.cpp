...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 DMS irreversibly.
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in dms.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("dms.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in dms.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* DMS features */
    { "dms",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "dms",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "dms",               "gobject",                &gobject,                {} },
    { "dms",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
