 ...
 * between dmsd, and dms-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<dmsaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by DMS Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
std::string LicenseInfo()
{
    ...
    return CopyrightHolders(_("Copyright (C)"), 2014, COPYRIGHT_YEAR) + "\n" +
           ...
           "Responsible Person i. t. o. p. r § 18 pars: 2 MStV: Harald Krekeler (Address see above)\n" +
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if DMS Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if DMS Core is started while in a temporary directory.\n",
