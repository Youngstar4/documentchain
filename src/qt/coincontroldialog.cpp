...
void CoinControlDialog::lockCoin()
{
    ...
    if (act == lockPermanentAction) {
        QString unspent = contextMenuItem->data(COLUMN_ADDRESS, TxHashRole).toString() + " " + contextMenuItem->data(COLUMN_ADDRESS, VOutRole).toString();
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::DMS;
    ...
    QString toolTip4 = tr("Can vary +/- %1 docz(s) per input.").arg(dFeeVary);
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if (ExtractDestination(out.txout.scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show dms address. In tree mode, address is not shown again for direct wallet address outputs
