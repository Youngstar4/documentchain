...
port 9050 and only allows .onion nodes to connect:
	...
	./dmsd -onion=127.0.0.1:9050 -onlynet=tor -listen=0 -addnode=ssapp53tmftyjmjb.onion
...
your dmsd's P2P listen port (9999 by default).
	...
	-externalip=X   You can tell DMS Core about its publicly reachable address using
	                ...
	                /var/lib/tor/dmscore-service/hostname. Onion addresses are given
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./dmsd -proxy=127.0.0.1:9050 -externalip=ssapp53tmftyjmjb.onion -listen
...
DMS Core has been updated to make use of this.

This means that if Tor is running (and proper authentication has been configured),
DMS Core automatically creates a hidden service to listen on. This will positively 
affect the number of available .onion nodes.

This new feature is enabled by default if DMS Core is listening (`-listen`), and
requires a Tor connection to work. It can be explicitly disabled with `-listenonion=0`
and, if not disabled, configured using the `-torcontrol` and `-torpassword` settings.
To show verbose debugging information, pass `-debug=tor`.

Connecting to Tor's control socket API requires one of two authentication methods to be 
configured. For cookie authentication the user running dmsd must have write access 
to the `CookieAuthFile` specified in Tor configuration. In some cases this is 
preconfigured and the creation of a hidden service is automatic. If permission problems 
are seen with `-debug=tor` they can be resolved by adding both the user running tor and 
the user running dmsd to the same group and setting permissions appropriately. On 
Debian-based systems the user running dmsd can be added to the debian-tor group, 
