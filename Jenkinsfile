...
// which are part of the DMS repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="dms-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /dms-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "dashpay-dms/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from dashpay-dms/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'dashpay-dms/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from dashpay-dms/develop"
            }
          }

          if (hasCache) {
            sh "cd /dms-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /dms-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /dms-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /dms-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /dms-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /dms-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /dms-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
